...
void cplug_process(void *ptr, CplugProcessContext *ctx) {
  ...
  // Audio thread has chance to respond to incoming GUI events before being sent
  // to the host
  int head =
      cplug_atomic_load_i32(&plugin->mainToAudioHead) & CPLUG_EVENT_QUEUE_MASK;
  ...
  while (ctx->dequeueEvent(ctx, &event, frame)) {
    ...
    case CPLUG_EVENT_PROCESS_AUDIO: {
      // If your plugin does not require sample accurate processing, use this
      // line below to break the loop frame = event.processAudio.endFrame;
      ...
      if (plugin->midiNote == -1) {
        ...
        float Hz =
            440.0f * exp2f(((float)plugin->midiNote - 69.0f) * 0.0833333f);
...
void cplug_loadState(void *userPlugin, const void *stateCtx,
  ...
  // If your plugin has added/removed parameters, requesting for more data then
  // you actually expect may be a good idea
